...
class User:
    ...
    def logout_post(self):
        ...
        headers = {
            'X-OBSERVATORY-AUTH': self.apikey
        }
    ...
    def useCase_get(self, url, reqAuthorization=False, form='json'):
        ...
        if form == 'json':
            ...
            #prints all columns => format lost
            #pandas.options.display.width = 0
            pandas.set_option('display.max_rows', None)
            df = pandas.read_csv(str_buffer, index_col=0)
            print(df)
        else:
...
class Admin(User):
    def __init__(self, params, home=None):
        super().__init__(home)
        self.params = params
        self._token = self.params['apikey']
    ...
    def sessionsupd_post(self):
        ...
        click.echo(f"{chunks_created} file chunks have been created")
